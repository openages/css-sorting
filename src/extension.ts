...
import * as sorter from './css-sorting'
...
const configProfiler = new ConfigProfiler(null, {
	...
	configFiles: ['css-sorting.js', 'css-sorting.json', '.css-sorting.js', '.css-sorting.json'],
	envVariableName: 'CSS_SORTING_CONFIG',
	props: {
		package: 'cssSortingConfig'
	}
})
...
function use(settings: ISettings, document: vscode.TextDocument, range: vscode.Range): Promise<IResult> {
	return getConfigForFile(document, settings.config).then(config =>
		!config ? null : sorter.use(config, document, range)
	)
}
